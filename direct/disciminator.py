...
class Discriminator(th.nn.Module): 
  ...
  def __init__(
    ...
    """Construct discriminator network. Args:
      env: the used environment (used to calculate the network dimensions & normalize batches of training data)
      ...
      optim_kwargs: additional parameters to be passed the optimizer defaults to lower lr, as suggested for gans
      activation: activation to apply after hidden layers.
      use_obs, use_act, use_rew: should the current observation/action/reward be included as an input to the MLP
  ...
  def forward(self, obs:th.Tensor, act:th.Tensor, rew:th.Tensor) -> th.Tensor:
    """Compute the discriminator's logits for a state-action-reward sample. Args:
        obs: observation at time t. act: action taken at time t. 
    ...
    Returns: preds: discriminative predictions for given activation (default: ReLU)
        high (>0) -> current policy (generator) | low (<=0) -> imitation buffer (expert) """ 
    inputs = []
  ...
  def prepare_batch(self, policy_buffer: DirectBuffer, direct_buffer: DirectBuffer) -> Dict[str, th.Tensor]:
    """ Build and return training batch for the next discriminator update. Args:
      direct_buffer: Buffer to sample expert. policy_buffer: Buffer to sample policy.
